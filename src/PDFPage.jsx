    ...
    if (affectedTokens.length > 0 && containerRef.current) {
        affectedTokens.forEach(t => {
            t.parts.forEach(p => {
                ...
                overlay.style.left = `${(p.bounds.left - padding).toFixed(2)}px`;
                overlay.style.top = `${(p.bounds.top - padding).toFixed(2)}px`;
                overlay.style.width = `${(p.bounds.width + padding * 2).toFixed(2)}px`;
                overlay.style.height = `${(p.bounds.height + padding * 2).toFixed(2)}px`;
